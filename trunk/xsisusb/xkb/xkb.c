...
XkbSizeVirtualModMap(XkbDescPtr xkb,xkbGetMapReply *rep)
{
    ...
    for (nRtrn=i=0;i<rep->nVModMapKeys;i++,pMap++) {
	if (*pMap != 0)
	    nRtrn++;
    }
...
ProcXkbGetKbdByName(ClientPtr client)
{
	...
	*xkb->ctrls= *old_xkb->ctrls;
	...
	xkb->ctrls->num_groups= nTG;
        ...
        for (tmpd = inputInfo.devices; tmpd; tmpd = tmpd->next) {
            /* Copy the keymap if:
             *  - this is the device we wanted, or
             *  - this is the core device, we're setting the map on an
             *    extended device, which is currently being used as the core
             *    device, or
             *  - we're setting the keymap on the core device, and this is
             *    an extended device sending core events. */
            if (tmpd == dev ||
                (tmpd->id == inputInfo.keyboard->id &&
                 GetCoreKeyboard() == dev) ||
