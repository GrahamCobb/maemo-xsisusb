...
ephyrPoll(void)
{
  ...
  while (hostx_get_event(&ev))
	...
	case EPHYR_EV_MOUSE_PRESS:
	  ...
	  KdEnqueuePointerEvent(ephyrMouse, mouseState, ev.data.mouse_down.x,
                                ev.data.mouse_down.y, 0);
	  break;
	...
	case EPHYR_EV_MOUSE_RELEASE:
	  ...
	  KdEnqueuePointerEvent(ephyrMouse, mouseState, ev.data.mouse_up.x,
                                ev.data.mouse_up.y, 0);
	  break;
...
MouseInit (KdPointerInfo *pi)
{
    ...
    pi->nAxes = 3;
    pi->nButtons = 32;
    pi->common.name = KdSaveString("Xephyr virtual mouse");
    ephyrMouse = pi;
    return Success;
}
...
EphyrKeyboardInit (KdKeyboardInfo *ki)
{
  ...
  ki->common.name = KdSaveString("Xephyr virtual keyboard");
  ephyrKbd = ki;
  return Success;
}
