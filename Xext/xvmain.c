...
static int
XvdiDestroyGrab(pointer data, XID id)
{
  XvPortPtr pPort = data;

  if (pPort->pDraw)
    XvdiStopVideo(pPort->grab.client, pPort, pPort->pDraw);
...
XvdiGrabPort(
  ...
  if (!AddResource(id, XvRTGrab, pPort))
    {
      return BadAlloc;
    }
...
XvdiUngrabPort(
  ...
  if (pPort->pDraw)
    {
      (void) XvdiStopVideo(client, pPort, pPort->pDraw);
    }
