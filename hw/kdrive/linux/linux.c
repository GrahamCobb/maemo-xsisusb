...
static void
LinuxSwitchVT (int absrel, int num)
{
    struct vt_stat vts = {0};

    ioctl(LinuxConsoleFd, VT_GETSTATE, &vts);

    if (absrel == KD_VT_RELATIVE)
        num = vts.v_active + num;

    if (num != vts.v_active && (vts.v_state & (1 << num)))
        ioctl (LinuxConsoleFd, VT_ACTIVATE, num);
